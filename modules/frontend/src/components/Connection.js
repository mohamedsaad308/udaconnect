...
class Connection extends Component {
  ...
  getConnections = (personId) => {
    if (personId) {
      ...
      fetch(
        `${process.env.CONNECTIONS_API}/api/persons/${personId}/connection?start_date=2020-01-01&end_date=2020-12-30&distance=5`
      )
      ...
      <div className="connectionBox">
        ...
        <ul className="connectionList">
          {this.state.connections
            .filter((value, index, a) => a.findIndex((v) => v.person.id === value.person.id) === index)
